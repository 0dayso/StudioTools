...
public partial class Database_Output_SaleShop_payment_Callback : GetTogether.Web.UI.PageCallback
{
    ...
    {
        Database_Output_SaleShop_payment_Result c = (Database_Output_SaleShop_payment_Result)Page.LoadControl("SaleShop_payment_Result.ascx");
        DO_SaleShop_payment.UO_SaleShop_payment l = new DO_SaleShop_payment.UO_SaleShop_payment();
        GetTogether.Web.WebHelper.SetValues<DO_SaleShop_payment.UO_SaleShop_payment>(l, "SaleShop_payment_");
        if (string.IsNullOrEmpty(Sort)) Sort = DO_SaleShop_payment.Columns.pay_id.ToString();
        PagingResult<DO_SaleShop_payment.UO_SaleShop_payment, DO_SaleShop_payment.UOList_SaleShop_payment> resultPaging = BO_SaleShop_payment.GetPagingList(l, PageIndex, PageSize, Sort, IsAsc);
        c.JsFunction = "SaleShop_payment_Search";
        c.SetData<DO_SaleShop_payment.UO_SaleShop_payment, DO_SaleShop_payment.UOList_SaleShop_payment>(resultPaging.Result);
    ...
    private void Delete()
    {
        try
        {
            ...
            int[] ids = GetParameterArrayInt(Request["keys"], "SaleShop_payment_", ',');
            ParameterCollection conditions = new ParameterCollection();
            conditions.AddCondition(ParameterType.Initial, TokenTypes.In, DO_SaleShop_payment.Columns.pay_id, ids);
 
            if (new DO_SaleShop_payment().Delete(conditions) > 0)
                JsonSuccess();
            else
