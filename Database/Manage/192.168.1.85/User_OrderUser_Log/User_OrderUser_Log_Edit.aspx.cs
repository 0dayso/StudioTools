...
public partial class Database_Output_User_OrderUser_Log_Edit : GetTogether.Web.UI.Page
{
    public DO_User_OrderUser_Log.UO_User_OrderUser_Log UO;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (id > 0)
        {
            UO = BO_User_OrderUser_Log.GetObjectById(id);
        }
        if (UO == null) UO = new DO_User_OrderUser_Log.UO_User_OrderUser_Log();

        if (Request.Form.Keys.Count > 0)
        {
            GetTogether.Web.WebHelper.SetValues<DO_User_OrderUser_Log.UO_User_OrderUser_Log>(UO, "User_OrderUser_Log_");
            if (id > 0)
            {
				//Pending
				UO["UpdateOn","yyyy-MM-dd"]=DateTime.Now.ToString("yyyy-MM-dd");
				UO["UpdateBy"] = string.Concat("WS-",Request.UserHostAddress);
                UO.Update(BO_User_OrderUser_Log.GetConditionsById(id));
            }
            else
            {
				//Pending
            ...
            Response.Redirect("User_OrderUser_Log.aspx");
        }
    }
