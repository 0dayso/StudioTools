...
public partial class Database_Output_SaleShop_ArticleCate_Callback : GetTogether.Web.UI.PageCallback
{
    ...
    private void GetList()
    {
        ...
        if (string.IsNullOrEmpty(Sort)) Sort = DO_SaleShop_ArticleCate.Columns.Id.ToString();
    ...
    private void Delete()
    {
        try
        {
            ...
            int[] ids = GetParameterArrayInt(Request["keys"], "SaleShop_ArticleCate_", ',');
            ParameterCollection conditions = new ParameterCollection();
            conditions.AddCondition(ParameterType.Initial, TokenTypes.In, DO_SaleShop_ArticleCate.Columns.Id, ids);
 
            if (new DO_SaleShop_ArticleCate().Delete(conditions) > 0)
                JsonSuccess();
            else
