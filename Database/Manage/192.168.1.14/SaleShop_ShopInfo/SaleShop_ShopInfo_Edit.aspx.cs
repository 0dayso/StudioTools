...
public partial class Database_Output_SaleShop_ShopInfo_Edit : GetTogether.Web.UI.Page
{
    public DO_SaleShop_ShopInfo.UO_SaleShop_ShopInfo UO;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (id > 0)
        {
            UO = BO_SaleShop_ShopInfo.GetObjectById(id);
        }
        if (UO == null) UO = new DO_SaleShop_ShopInfo.UO_SaleShop_ShopInfo();

        if (Request.Form.Keys.Count > 0)
        {
            GetTogether.Web.WebHelper.SetValues<DO_SaleShop_ShopInfo.UO_SaleShop_ShopInfo>(UO, "SaleShop_ShopInfo_");
            if (id > 0)
            {
				//Pending
				UO["UpdateOn","yyyy-MM-dd"]=DateTime.Now.ToString("yyyy-MM-dd");
				UO["UpdateBy"] = string.Concat("WS-",Request.UserHostAddress);
                UO.Update(BO_SaleShop_ShopInfo.GetConditionsById(id));
            }
            else
            {
				//Pending
            ...
            Response.Redirect("SaleShop_ShopInfo.aspx");
        }
    }
