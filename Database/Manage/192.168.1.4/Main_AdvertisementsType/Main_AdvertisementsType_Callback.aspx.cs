...
public partial class Database_Output_Main_AdvertisementsType_Callback : GetTogether.Web.UI.PageCallback
{
    ...
    private void GetList()
    {
        ...
        if (string.IsNullOrEmpty(Sort)) Sort = DO_Main_AdvertisementsType.Columns.Id.ToString();
    ...
    private void Delete()
    {
        try
        {
            ...
            int[] ids = GetParameterArrayInt(Request["keys"], "Main_AdvertisementsType_", ',');
            ParameterCollection conditions = new ParameterCollection();
            conditions.AddCondition(ParameterType.Initial, TokenTypes.In, DO_Main_AdvertisementsType.Columns.Id, ids);
 
            if (new DO_Main_AdvertisementsType().Delete(conditions) > 0)
                JsonSuccess();
            else
