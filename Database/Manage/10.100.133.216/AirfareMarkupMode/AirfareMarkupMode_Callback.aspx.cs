...
public partial class Database_Output_AirfareMarkupMode_Callback : GetTogether.Web.UI.PageCallback
{
    ...
    private void GetList()
    {
        ...
        if (string.IsNullOrEmpty(Sort)) Sort = DO_AirfareMarkupMode.Columns.ID.ToString();
    ...
    private void Delete()
    {
        try
        {
            ...
            int[] ids = GetParameterArrayInt(Request["keys"], "AirfareMarkupMode_", ',');
            ParameterCollection conditions = new ParameterCollection();
            conditions.AddCondition(ParameterType.Initial, TokenTypes.In, DO_AirfareMarkupMode.Columns.ID, ids);
 
            if (new DO_AirfareMarkupMode().Delete(conditions) > 0)
                JsonSuccess();
            else
