...
public partial class Database_Output_AirfareLastView_Callback : GetTogether.Web.UI.PageCallback
{
    ...
    private void GetList()
    {
        ...
        if (string.IsNullOrEmpty(Sort)) Sort = DO_AirfareLastView.Columns.Id.ToString();
    ...
    private void Delete()
    {
        try
        {
            ...
            int[] ids = GetParameterArrayInt(Request["keys"], "AirfareLastView_", ',');
            ParameterCollection conditions = new ParameterCollection();
            conditions.AddCondition(ParameterType.Initial, TokenTypes.In, DO_AirfareLastView.Columns.Id, ids);
 
            if (new DO_AirfareLastView().Delete(conditions) > 0)
                JsonSuccess();
            else
