...
public partial class Database_Output_APIS_Info_Callback : GetTogether.Web.UI.PageCallback
{
    ...
    private void GetList()
    {
        ...
        if (string.IsNullOrEmpty(Sort)) Sort = DO_APIS_Info.Columns.Id.ToString();
    ...
    private void Delete()
    {
        try
        {
            ...
            int[] ids = GetParameterArrayInt(Request["keys"], "APIS_Info_", ',');
            ParameterCollection conditions = new ParameterCollection();
            conditions.AddCondition(ParameterType.Initial, TokenTypes.In, DO_APIS_Info.Columns.Id, ids);
 
            if (new DO_APIS_Info().Delete(conditions) > 0)
                JsonSuccess();
            else
