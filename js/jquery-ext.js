...
function ConfirmAndDelSelCss(content, title, Url, type, chkPrefix, callBackParaName, divId, divBgId, divParId, inputPrefix, parAppend, dType, jsFunc, isChecked) {
    ...
    Confirm(content, title, true, divBgId, 'CallAjaxForDelSel(\'' + Url + '\',' + type + ',\'' + chkPrefix + '\',\'' + callBackParaName + '\',\'' + divId + '\',\'' + divBgId + '\',\'' + divParId + '\',\'' + inputPrefix + '\',\'' + parAppend + '\',\'' + dType + '\',\'' + jsFunc + '\',' + isChecked + ')', 'btn-y', 'btn-y');
}
...
function AjaxMsg(title, width, height, serUrl, page, sId, func, fixedTop) {
    var f = func;
    if (f == null) f = EndRequestAndValidate;
    MsgAjax(title, width, height, serUrl, page, sId, f, fixedTop);
}
...
function DateControljQuery(sId, butId, onchange, allowEmpty, left, top, error_lb, isAddErrCss, minDate, maxDate, numberOfMonths, isDateTime) {
    if (numberOfMonths == null) numberOfMonths = 2;
    ...
    if (isDateTime) {
        ...
        //RegJs(JsPath+"/jq/ui/jquery-ui-timepicker-addon.js");
    }
    else {
        //        RegJs(SerUrlValue+"js"+(FVersion==null?"":FVersion)+"/jquery/jquery.ui.datepicker-"+lang+".js");
        //RegJs(JsPath+"/jq/ui/jquery.ui.datepicker.js");
    }
    ...
    if (isDateTime) {
        ...
        $(oj).datepicker({
            ...
            left: left,
            top: top,
            buttonImage: SerUrlValueStyle + "images/calendar.png"
        });
    }
    var lv = oj.value;
    oj.onfocus = "";
...
var loading = "<img src='" + SerUrlValueStyle + "images/loading.gif' />";
var sloading = "<img src='" + SerUrlValueStyle + "images/s_loading.gif' style='border-width:0px;'/>";
var hloading = "<img src='" + SerUrlValueStyle + "images/hbload.gif' style='border-width:0px;'/>";
var errorImg = "<img src='" + SerUrlValueStyle + "images/problem.gif' style='padding:0px 5px 0px 5px;' />";
var ajaxLoading = "<img src='" + SerUrlValue + "images/ajax-loading.gif' style='padding:0px 5px 0px 5px;' />";
function SetupInit() {
    loading = "<img src='" + SerUrlValueStyle + "images/loading.gif' />";
    sloading = "<img src='" + SerUrlValueStyle + "images/s_loading.gif' style='border-width:0px;'/>";
    hloading = "<img src='" + SerUrlValueStyle + "images/hbload.gif' style='border-width:0px;'/>";
    errorImg = "<img src='" + SerUrlValueStyle + "images/problem.gif' style='padding:0px 5px 0px 5px;' />";
    ajaxLoading = "<img src='" + SerUrlValueStyle + "images/ajax-loading.gif' style='padding:0px 5px 0px 5px;' />";
}
...
Browser.isIE9 = $.browser.msie && $.browser.version == '9.0';
...
function ShowDiv(content, width, height, bordercolor, borderstyle, left, top, sId, fixedTop) {
    if (o("alert_div") != null) {
        CM();
    }
    ...
    if (bgObj != null && sId != null) {
        var sTop = GetPosition(bgObj).top + (GetPosition(bgObj).height - height) / 2 - 8;
        if (sTop > 300 && content.indexOf('loading-content') == -1 && (sId == null || sId == '')) sTop = 300;
    ...
    if (fixedTop > 0) {
        ...
        alert_div.style.top = fixedTop + 'px';
...
function MsgAjax(title, width, height, serUrl, page, sId, func, fixedTop) {
    if (IsEmpty(title)) title = "System Information";
    var content_body = "<table width='100%' border='0px' cellpadding='0px' cellspacing='0px'><tr class=\"msg-title\"><td align='left'>&nbsp;" + title +
    "</td><td align='right' style='text-align:right;'><div onclick='CloseMsgBox();' class='msg-cs'></div></td></tr><tr><td colspan='2' id='ajax-content' style='background-color:white;'><div style='text-align:center;width:100%;'><img src='" + SerUrlValueStyle + "images/loading.gif' /></div>" +
    "</td></tr></table>";
    a("ajax-content", serUrl + page, '', 'post', func == null ? EndRequest : func, true);
    ShowBackgroup(sId);
    ShowDiv(content_body, width, height, "#FFB97A", "MSGBOX", 0, 0, sId, fixedTop);
    ShowBackgroup(sId);
    sf("_MsgBtn");
}
...
function MsgBoxFun(is_confirm, content, title, is_error, sId, script_str, btn_ok_css, btn_cancel_css, script_cancel) {
    CM();
    if (IsEmpty(btn_ok_css)) btn_ok_css = "btn-y";
    if (IsEmpty(btn_cancel_css)) btn_cancel_css = "btn-y";
    if (IsEmpty(title)) title = "System Information";
    if (is_error) content = "<table width='100%'><tr><td align='center' style='padding-left: 10px; padding-right: 20px; width: 1px'><img src='" + SerUrlValueStyle + "images/error1.gif' /></td><td width='99%' align='left'>" + content + "</td></tr></table>";
    ...
    ShowBackgroup(sId);
    ShowDiv(content_body, 330, 130, "#FFB97A", "MSGBOX", 0, 0, sId);
    ShowBackgroup(sId);
    sf("_MsgBtn");
}
...
function sld2(sId) {
    CM();
    ShowLoading2(loading, sId);
}
function ShowLoading2(l, sId)//ShowLoading
{
    ShowBackgroup(sId, 0.2);
    ShowDiv(("<div>" + ajaxLoading + "<div class='loading-content'>" + GetMultiLang('PleaseWait') + "</div></div>"), 150, 70, "", "", 0, 0, sId);
}
function ShowLoading(l, sId)//ShowLoading
{
    ShowBackgroup(sId);
    ShowDiv(("<div class='loading'>" + ajaxLoading + "<div class='loading-content'>" + GetMultiLang('PleaseWait') + "</div></div>"), 160, 31, "", "", 0, 0, sId);
}
function ShowBackgroup(sId, opacity) {
    ...
    if (!Browser.isIE || Browser.isIE8) {
        ...
        bgObj.style.top = GetPosition(obj).top + 30 + "px";
        bgObj.style.left = GetPosition(obj).left + "px";
    }
    else {
        bgObj.style.top = GetPosition(obj).top + "px";
...
function ScrollToControl(id, abligate_height) {
    ScrollToControl2(document.getElementById(id), abligate_height)
}
function ScrollToControl2(elem, abligate_height) {
...
function setCK(name, value, expires, path, domain, secure) { if (expires == null) { expires = 360; } var today = new Date(); today.setTime(today.getTime()); if (expires) { expires = expires * 1000 * 60 * 60 * 24; } var expires_date = new Date(today.getTime() + (expires)); document.cookie = name + '=' + escape(value) + ((expires) ? ';expires=' + expires_date.toGMTString() : '') + ((path) ? ';path=' + path : '') + ((domain) ? ';domain=' + domain : '') + ((secure) ? ';secure' : ''); }
...
window.history.back = function (i) { if (i == null || i == 0) { i = 1; } if (history.length == 0) { window.location = SerUrlValue + 'Default.aspx'; } else { history.go(-i); } }
...
function RegRangeDateControl(fromTxtId, fromBtnId, toTxtId, toBtnId, dateNum, allowEmpty, enforce, minDate, maxDate, isDateTime, onChangeFuncOverwrite) {
    var obj = o(fromTxtId);
    ...
    var onChangeFunc = function (txtId) {
        if (onChangeFuncOverwrite != null && $.isFunction(onChangeFuncOverwrite)) {
            onChangeFuncOverwrite(txtId);
        }
        else {
...
var mtls_reserve = [["PleaseWait", "Please Wait..."], ["Details", "Details"]];
function GetMultiLang(key, data) {
    if (data == null && typeof (mtls) != 'undefined') data = mtls;
    else data = mtls_reserve;
...
function obc2sbcForIE(obj) {
    var pos = getCursorPos(obj);//保存原始光标位置
    var temp = obj.value;
    obj.value = temp.replace(/[\uff01-\uff5e]/g, function (a) { return String.fromCharCode(a.charCodeAt(0) - 65248); }).replace(/\u3000/g, " ");
...
function RangeDateControl(FId, TId, numberOfMonths) {
    if (numberOfMonths == null) numberOfMonths = 2;
    var dates = $("#" + FId + ", #" + TId).datepicker({
        //	    defaultDate: "+1w",
        changeMonth: true,
        changeYear: true,
        showOn: "button",
        ...
        onSelect: function (selectedDate) {
            var option = this.id == FId ? "minDate" : "maxDate",
...
function GetDeviceType() {
    ...
    else if (ua.match(/ipod os/i) == 'ipod os') {
        return 'iPod';
    }
    ...
    else if (ua.match(/windows nt 5.0/i) == 'windows nt 5.0') {
        return 'Windows 2000';
    }
    else if (ua.match(/windows nt 5.1/i) == 'windows nt 5.1') {
        return 'Windows XP';
    }
    else if (ua.match(/windows nt 5.2/i) == 'windows nt 5.2') {
        return 'Windows 2003';
    }
    else if (ua.match(/windows nt 6.0/i) == 'windows nt 6.0') {
        return 'Windows Vista';
    }
    else if (ua.match(/windows nt 6.1/i) == 'windows nt 6.1') {
        return 'Windows 7';
    }
    else if (ua.match(/windows nt 6.2/i) == 'windows nt 6.2') {
        return 'Windows 8';
    }
